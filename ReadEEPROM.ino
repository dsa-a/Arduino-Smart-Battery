...
/*
 * ReadEEPROM.ino (https://github.com/dsa-a/Arduino-Smart-Battery)
...
void setup() {
  ...
  Serial.println(F("Read EEPROM bq20z... IC"));
  ...
  for (byte i=0; i<(0x800/32); i++) {
    WriteSMBWord(0x09, 0x4000+i*32);
    delay(100);
    ReadBlockSMB(byte(0x0C));
    for (byte j=1; j<=32; j++) {
      printHEX(buff[j]);
      if (j!=32) Serial.print(" ");
    }
    Serial.println();
  }
